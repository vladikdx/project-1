...
window.addEventListener("load", async () => {
  ...
  let respDep = await fetch("https://localhost:44319/api/department/" + departmentID);
  let dataDep = await respDep.json();
  let pageTitle = document.getElementById("title");
  pageTitle.innerText = `Edit Department (${dataDep.Name})`;
  let respEmp = await fetch("https://localhost:44319/api/employeedepartment/");
  let dataEmp = await respEmp.json();
  let departmentName = (document.getElementById("depertmentName").value = dataDep.Name);
  let managerSelect = document.getElementById("depertmentManager");
  
  dataEmp.forEach((x) => {
    if(x.DepartmentName == dataDep.Name) {
      managerSelect[0].style.display = "none";
      managerSelect[1].style.display = "block";
      managerSelect.selectedIndex = 2;
    let managerOption = document.createElement("option");
    managerOption.innerText = x.FirstName;
    managerOption.value = x.ID;
...
saveBtn.addEventListener("click", async () => {
  ...
  let managerSelect = document.getElementById("depertmentManager").value;
  updateAction(userID, todayDate);
  let obj = { Name: departmentName, Manager: managerSelect };
  let fetchParams = {
    method: "PUT",
    body: JSON.stringify(obj),
    headers: { "Content-Type": "application/json" },
  };
  let respDep = await fetch("https://localhost:44319/api/department/" + departmentID, fetchParams);
