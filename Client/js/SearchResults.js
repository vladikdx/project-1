...
window.addEventListener("load", async () => {
  ...
  let resp = await fetch("https://localhost:44319/api/employeedepartment/?search=" + searchQuary);
  ...
  pageTitle.innerText = `Search Results: ${searchQuary}(${data.length})`;
  ...
  data.forEach((x) => {
    ...
    if (x.Manager != null) {
      ...
      shiftsItem.innerText = `Manager of ${x.Manager}`;
      managerList.appendChild(shiftsItem);
      managerTdObj.appendChild(managerList);
    ...
    if (x.Shifts.length > 0) {
      x.Shifts.forEach((j) => {
        let shiftsItem = document.createElement("li");
        shiftsItem.innerText =  `${j.Date.slice(0, 10)} Start: ${j.StartTime} End: ${j.EndTime}   `;
...
async function editEmployeeRedirect(employeeID) {
